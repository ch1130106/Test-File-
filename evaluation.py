...
response = cv2.imread('response.jpg')
...
for y in range(0,len(response_d)):
	for x in range(0,len(akey_d)):
		if abs(response_d[y][0]-akey_d[x][0])<6:
			if abs(response_d[y][1]-akey_d[x][1])<6:
				if abs(response_d[y][2]-akey_d[x][2])<6:
					if abs(response_d[y][3]-akey_d[x][3])<6:
						pmarks=pmarks+4
